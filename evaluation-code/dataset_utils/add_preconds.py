...
def get_preconds_script(script_lines, verbose=False):
    ...
    for i in range(len(content)):
        ...
        if action.upper() == 'PLUGOUT':
            if obj_id in is_plugged.keys() and not is_plugged[obj_id]:
                ...
                if verbose:
                    print('\n'.join(content))
                raise ScriptFail('Error, {} already plugged out'.format(obj_names[0]))
            else:
        ...
        if action.upper() == 'PLUGIN':
            if obj_id not in is_plugged.keys():
                ...
                if verbose:
                    print('\n'.join(content))
                raise ScriptFail('Error, {} plugged in twice'.format(obj_names[0]))
    ...
    for i in range(len(content)):
        ...
        if action.upper() == 'SWITCHOFF':
            if obj_id not in is_on.keys(): # If this light was never switched on/off
                ...
                # CHANGED: catch error when obj does not exist in knowledge base
                try:
                    ...
                    raise ScriptFail('Error, {} not found in properties'.format(obj_id[0]))

            else:
                if not is_on[obj_id]:
                    if verbose:
                        print('\n'.join(content))
                    raise ScriptFail('Error, {} turned off twice'.format(obj_names[0]))
            is_on[obj_id] = False
        ...
        if action.upper() == 'SWITCHON':
            if obj_id in is_on.keys() and is_on[obj_id]:
                if verbose:
                    print('\n'.join(content))
                raise ScriptFail('Error, {} turned on twice'.format(obj_names[0]))
    ...
    for i in range(len(content)):
        ...
        if action.upper() in ['RUN', 'WALK']:
            ...
            if object_id not in object_grabbed.keys() or not object_grabbed[object_id]:
                # CHANGED: if we last walked to a room, assume we are close to new object
        ...
        if action.upper() == 'PUTOFF':
            if object_id not in puton.keys():
                ...
                if not puton[object_id]:
                    if verbose:
                        print('\n'.join(content))
                    raise ScriptFail('Error, {} was already off'.format(obj_names[0]))
